...
for (0 => int i; i < channels; i++)
{
    ...
    amen[i] => pan[i] => dac;
    //amen[i].chan(0) => lowpass[i] => hipass[i] => pan[i].chan(0) => dac.chan(0);
    //amen[i].chan(1) => lowpass[i+channels] => hipass[i+channels] => pan[i].chan(1) => dac.chan(1);
    
    //10::ms => now;
    if (i < 4) {
        pan[i] => recordings[0] => blackhole;
        //pan[i].chan(0) => recordings[0].chan(0) => blackhole;
        //pan[i].chan(1) => recordings[0].chan(1) => blackhole;
    }
    if (i > 3 && i < 6) {
        pan[i] => recordings[1] => blackhole;
        //pan[i].chan(0) => recordings[1].chan(0) => blackhole;
        //pan[i].chan(1) => recordings[1].chan(1) => blackhole;
    }
    if (i > 5 && i < 10) {
        pan[i] => recordings[2] => blackhole;
        //pan[i].chan(0) => recordings[2].chan(0) => blackhole;
        //pan[i].chan(1) => recordings[2].chan(1) => blackhole;
    }
    if (i > 9) {
        pan[i] => recordings[3] => blackhole;
        //pan[i].chan(0) => recordings[3].chan(0) => blackhole;
        //pan[i].chan(1) => recordings[3].chan(1) => blackhole;
    }
    //10::ms => now;
    pan[i] => recordings[4] => blackhole;
    //pan[i].chan(0) => recordings[4].chan(0) => blackhole;
    //pan[i].chan(1) => recordings[4].chan(1) => blackhole;
